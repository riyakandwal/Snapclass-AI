...
from src.components.dialogue_create_sub import create_subject_dialog
from src.components.dialogue_share_sub import share_subject_dialog
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username=st.text_input("Enter username",placeholder='Ex:ananyaroy')
    teacher_pass=st.text_input("Enter the password",type="password",placeholder='Enter password ')
    st.divider()

    btcl1,btcl2=st.columns(2)
...
def register_teacher(teacher_username,teacher_pass,teacher_name,teacher_pass_confirm):
   ...
   try:
      ...
      return True,"Successfully created account,Loginn now:)"
...
def teacher_screen_register():
    ...
    with c2:
       if st.button("Go back to Home ",type="secondary",key='loginbackbtn',shortcut="control+backspace"):
          st.session_state['login_type']=None
          st.rerun()
    st.header("Register your teacher's account",text_alignment='center')
    st.space()
    st.space()
    teacher_username=st.text_input("Enter username",placeholder='Ex:ananyaroy')
    teacher_name=st.text_input("Enter name",placeholder='Ex:ananya roy')
    teacher_pass=st.text_input("Enter the password",type="password",placeholder='Enter password ')
    teacher_pass_confirm=st.text_input("Confirm the password",type="password",placeholder="Enter password")
    st.divider()
